...
if [ -f /usr/local/bin/cbsd ]; then
  ...
  sudo cbsd jexec user=devel jname=pyserback env OFFLINE=${OFFLINE} /usr/src/bin/default_user.sh
  sudo tmux new-session -s "tilda" -d "cbsd jexec user=devel jname=pyserback env OFFLINE=${OFFLINE} /usr/src/bin/devel.sh"
  sudo tmux split-window -h -p 50 -t 0 "cbsd jexec user=devel jname=pyserfront env OFFLINE=${OFFLINE} /usr/src/bin/devel.sh"
  sudo tmux a
else
